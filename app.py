...
st.markdown("AI-based system for analyzing student performance risk")
...
if st.button("🚀 Predict Risk"):
    ...
    with col1:
        ...
        if pred == 2:
            st.error("🔴 HIGH RISK STUDENT (RED ALERT)")
            st.markdown("Immediate attention required")
        elif pred == 1:
            st.warning("🟡 MEDIUM RISK STUDENT (WARNING)")
            st.markdown("Needs improvement")
        else:
            st.success("🟢 LOW RISK STUDENT (SAFE)")
...
st.sidebar.write("Model: Random Forest + Risk Levels (Green/Yellow/Red)")